...
void CudaSpace::AddBody(const BodyData& body)
{
	std::unique_lock lockMutex(_mutex);
	_conditionMutex.wait(lockMutex, [this]() { return _countProcessing.load() == 0; });
...
void CudaSpace::AddBodies(const std::vector<BodyData>& bodies)
{
	std::unique_lock lockMutex(_mutex);
	_conditionMutex.wait(lockMutex, [this]() { return _countProcessing.load() == 0; });
...
void CudaSpace::Bodies(std::vector<GravityRender::Body>& bodies)
{
...
std::vector<BodyData> CudaSpace::GetBodies()
{
...
void CudaSpace::UpdateForce(size_t iBegin, size_t iEnd, size_t size)
{
...
void CudaSpace::UpdateForceHalf(size_t iBegin, size_t iEnd, size_t size)
{
	for (size_t i = iBegin; i < iEnd; ++i) {
		auto& bodyForce = _bodies[i].force;
...
void CudaSpace::UpdateForceHalfNoMutex(size_t iBegin, size_t iEnd, size_t size)
{
	for (size_t i = iBegin; i < iEnd; ++i) {
		auto& bodyForce = _bodies[i].force;
...
void CudaSpace::UpdatePositions()
{

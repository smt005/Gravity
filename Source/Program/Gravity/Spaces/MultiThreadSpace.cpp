...
void MultiThreadSpace::Update()
{
	...
	if (SpaceManager::paramC) LOG("MULTI [{}, {}] MAIN: {} SUB: {} countThread: {}", SpaceManager::paramA, SpaceManager::paramB, std::this_thread::get_id(), th.get_id(), std::thread::hardware_concurrency());
	th.detach();
}
		...
		if (!SpaceManager::paramA) {
			...
			if (!SpaceManager::paramB) {
				for (const auto& rengePair : ranges) {
					...
					if (SpaceManager::paramC) LOG("   sub thread: {}", th.get_id());
					th.join();
				}
			}
			else {
				for (const auto& rengePair : ranges) {
					...
					if (SpaceManager::paramC) LOG("   sub thread: {} NO MUTEX", th.get_id());
					th.join();
				}
			}
		}
